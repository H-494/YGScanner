...
public class SmddqrActivity extends AppCompatActivity {
    @BindView(R.id.tab)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_smddqr);
        setTitle("扫描倒剁确认");
    ...
    private void initFragment() {
        Smddqr s = new Smddqr();
        fragments[0] = s;
        SmddqrDelete sd = new SmddqrDelete();
        fragments[1] = sd;
    }
    ...
    public void getSum() {
        Cursor cur1 = db.rawQuery(
                "SELECT ifnull(sum(realwtg),0) as realwtg ,ifnull(sum(theoWgt),0) as realwtg,count(*) as js,ifnull(loadno,' ') FROM YG_DDXXM ",
                null);
        int x = cur1.getCount();
        Log.d("查询结果:", x + "");
        if (x != 0) {
    ...
    private void deleteAll() {
        ...
        ((Smddqr) fragments[0]).clear();
        ((SmddqrDelete) fragments[1]).clear();
    }
    ...
    class qurenkcddinfoHandler implements Runnable {
        ...
        public void run() {
            ...
            Cursor cur = db.rawQuery("SELECT invid FROM YG_DDXXM ", null);
            String hhhh1 = "";
            int i = 0;
            if (cur.getCount() != 0) {
            ...
            int w = 20 * i;
            List<String> list = getdd();
            String data = String.format("%-10s", "SHHL38") + "A"
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(SmddqrActivity.this);
                    builder.setTitle("提示")
                ...
                case 1:
                    ...
                    AlertDialog.Builder builder1 = new AlertDialog.Builder(SmddqrActivity.this);
    ...
    private ArrayList<String> getdd() {
