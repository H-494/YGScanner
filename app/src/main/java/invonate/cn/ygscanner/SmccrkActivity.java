        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(SmccrkActivity.this);
                    builder.setTitle("提示")
                            .setMessage(str)
    ...
    class querykcddinfoHandler implements Runnable {
        ...
        public void run() {
            ...
            if (!(scannerhwtmtext.contains("#"))) {
                data = String.format("%-10s", "SHHL34") + "A"
                        + String.format("%-20s", e) + "*";
            } else {
                data = String.format("%-10s", "SHHL34") + "A"
                        + String.format("%-51s", e) + "*";
...
//                        String query = "select * from  YG_FHXXM where INVID = "+invId;
                        Cursor cursor = db.rawQuery("SELECT * FROM YG_FHXXM WHERE INVID=?", new String[]{invId});
